...
if (empty($errors)) {
    ...
    try {
        ...
        $db->beginTransaction();
        /* master record*/
        $masterid = $db->insert('INSERT INTO pm_master (display_id,filename,cas_no,name,bibtex_ref_key,bibtex_key,bibtex_year,model_type,lj,charge,dipole,quadrupole,memory_loc,description,type,disp_sh,user_mode,ls1,ms2,lam,gro) 
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            array($dis_id, $substance, $casno, $name, 0, '-', 0, $modeltype, $_POST['lj'], $_POST['charge'], $_POST['dipole'], $_POST['quadrupole'], 'img/profile/' . $profileName, $description, $type, $disp_sh, $user_mode, $ls1, $ms2, $lam, $gro));
