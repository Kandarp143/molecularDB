...
//declare
    ...
    try {
        // update data
        $db = new Database();
        $db->update('UPDATE pm_master SET display_id=?,filename = ?,cas_no =?,name = ?,model_type= ?,lj= ?,charge= ?, dipole= ?, quadrupole= ?, description = ?,type=?,disp_sh= ? 
WHERE master_id= ?', array($display_id, $substance, $casno, $name, $modeltype, $_POST['lj'], $_POST['charge'], $_POST['dipole'], $_POST['quadrupole'], $description, $type, $disp_sh, $master_id));
